...
class Tmp : AppCompatActivity(), Detector.DetectorListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val detectButton =
            findViewById<com.google.android.material.floatingactionbutton.FloatingActionButton>(R.id.buttonDetect)
    ...
    private fun toggleCameraOrientation() {
        isFrontCamera = !isFrontCamera  // Đảo giá trị của isFrontCamera

        // Thay đổi lại selector của camera để chọn camera trước/sau
        ...
        // Cập nhật lại các use case của camera sau khi thay đổi camera
        camera?.let {
            it.cameraControl.cancelFocusAndMetering() // Hủy bỏ việc lấy nét cũ (nếu có)
            cameraProvider?.unbindAll() // Hủy liên kết camera trước đó
        }

        // Sau khi thay đổi camera selector, khởi động lại camera
    ...
    private fun startCameraFunctionality() {
        ...
        val rotateIcon: ImageView = findViewById(R.id.rotate_icon)


        rotateIcon.setOnClickListener {
            // Yêu cầu thay đổi hướng màn hình
...
//    }
    private fun startCamera() {
        ...
        cameraProviderFuture.addListener({
            ...
            // Xác định camera cần sử dụng (trước hoặc sau)
            val cameraSelector = if (isFrontCamera) {
                CameraSelector.Builder().requireLensFacing(CameraSelector.LENS_FACING_FRONT).build()
            } else {
                CameraSelector.Builder().requireLensFacing(CameraSelector.LENS_FACING_BACK).build()
            ...
            // Liên kết các use case với camera đã chọn
            bindCameraUseCases(cameraSelector)
        }, ContextCompat.getMainExecutor(this))
    }
    ...
    private fun bindCameraUseCases(cameraSelector: CameraSelector) {
        ...
        imageAnalyzer?.setAnalyzer(cameraExecutor) { imageProxy ->
            ...
            val matrix = Matrix().apply {
                ...
                // Nếu là camera trước, cần phải lật ảnh lại
                if (isFrontCamera) {
